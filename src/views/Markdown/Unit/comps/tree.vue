<template>
  <div class="paths" v-show="mdList.length">
...
const toCateTree = (list: any) => {
  ...
  list.forEach((item: any) => {
    let p = {}
    let length = item.paths.length
    deeps = deeps > length ? deeps : length
    let treeItem = (num: number) => {
      let pt = <any>{
        name: item.paths[num]
      }
      if (num + 1 !== length) {
...
//   console.log('深度化', showList)
  ...
  let joinDeepArr = (list: any) => {
    let itemArr = <any>[]
    list.forEach((item: any) => {
      let itemIndex = itemArr.findIndex((x: any) => x.name == item.name)
      if (itemIndex == -1) {
        itemArr.push(item)
