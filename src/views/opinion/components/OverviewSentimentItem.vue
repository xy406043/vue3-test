<template>
  <a-card style="border-top: 0; border-radius: 0px" :bodyStyle="{ padding: '24px 0px' }">
    <share-chart
    ...
    <div style="padding: 0px 24px">
      ...
      <s-alert
        title="笔记作者粉丝量级"
        desc="粉丝数<1000为素人，粉丝数1000-5000为KOC，粉丝数≥5000为KOL"
      />
      <share-chart
        style="padding: 0px 24px"
      ...
      <a-radio-group v-model:value="choseNoteType4Table" style="margin: 20px 0px">
        <a-radio-button :value="note.value" v-for="note in noteTypes4Table" :key="note.value">
          {{ note.label }}
        </a-radio-button>
      </a-radio-group>
      <a-table
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const category = ref<string>('positive'); // 监听切换分类状态
    ...
    const secondLineOptions = reactive<any>({ title: '不同情感倾向的笔记数量趋势' });

    watch([category, reportId], () => {
      useShareChartData(
        firstPieOptions,
        firstLineOptions,
        sourceData.value.sentiment.total,
        sourceData.value.sentiment.byDayList,
      ...
      // 笔记作者粉丝数量
      const secondData = sourceData.value[currentCategory].authorFans;
      const currentNoteType = noteTypes4Tab.find((item) => item.value === currentCategory);
    ...
    watch(
      [choseNoteType4Table, category, sourceData],
      () => {
        console.log(choseNoteType4Table, category, sourceData);
        const target = sourceData.value[category.value];
        tableData.value = target ? target.notes[choseNoteType4Table.value] : [];
      },
      { immediate: true },
    );
