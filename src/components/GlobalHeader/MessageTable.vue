  ...
  <div>
    ...
    >
      <span class="flex items-center">
        <span :class="!msg.hasView ? 'no-view' : ''">{{ msg.instName }}+{{ msg.keyword }}</span>
        <LoadingOutlined style="margin-left: 10px" v-if="msg.status == SearchStatus.Loading" />
      </span>
      <span class="flex">
        <span>{{ transformStatus(msg.status) }}</span>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      [currentPage, list],
      ([page, newList]) => {
        currentList.value = newList.slice((page - 1) * pageSize.value, page * pageSize.value);
      },
      { immediate: true },
    );
    return {
  ...
  methods: {
    transformStatus(status: SearchStatus) {
      return searchStatusCn[status];
    },

    async handleSeachResult(msg: any) {
      if (msg.status === SearchStatus.Loading) return message.warning('正在搜索中，请稍后');
